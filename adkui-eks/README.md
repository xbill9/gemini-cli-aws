...
- **Multi-Cloud Deployment**: Scripts for deploying agents to **Google Cloud Run** and **Amazon EKS**.
...
- `make deploy`: Deploys the application to Amazon EKS.
...
- `make status`: Retrieves the public LoadBalancer endpoint for the EKS service.
...
### Amazon EKS
To deploy the project to Amazon EKS:

1. **Create the cluster** (if not already exists):
   ```bash
   make cluster-create
   ```
