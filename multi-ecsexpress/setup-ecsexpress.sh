#!/bin/bash
# setup-ecsexpress.sh - Provision AWS ECS Express infrastructure for AI Course Creator
...
CLUSTER_NAME="adk-ecsexpress-cluster"
SERVICE_NAME="adk-ecsexpress-service"
...
SG_ID=$(aws ec2 describe-security-groups --filters "Name=vpc-id,Values=$VPC_ID" "Name=group-name,Values=adk-ecsexpress-sg" --query 'SecurityGroups[0].GroupId' --output text)

if [ "$SG_ID" == "None" ] || [ -z "$SG_ID" ]; then
    SG_ID=$(aws ec2 create-security-group --group-name "adk-ecsexpress-sg" --description "ADK ECS Express Security Group" --vpc-id "$VPC_ID" --query 'GroupId' --output text)
...
TASK_DEF_JSON=$(cat <<EOF
{
  ...
  "requiresCompatibilities": ["ECSEXPRESS"],
  "cpu": "1024",
  "memory": "2048"
}
EOF
