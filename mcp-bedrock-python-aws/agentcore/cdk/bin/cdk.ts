...
async function main() {
  ...
  const specPath = path.join(configRoot, 'agentcore.json');
  const specRaw = JSON.parse(fs.readFileSync(specPath, 'utf8'));

  // Transform artifact -> build for compute runtimes to satisfy CDK schema
  if (specRaw.agentCoreGateways) {
    for (const gateway of specRaw.agentCoreGateways) {
      for (const target of gateway.targets) {
        if (target.compute?.runtime?.artifact) {
          target.compute.runtime.build = target.compute.runtime.artifact;
          delete target.compute.runtime.artifact;
        }
      }
    }
  }
