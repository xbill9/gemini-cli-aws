...
if os.path.isdir(FRONTEND_DIST):
    logger.info(f"FRONTEND_DIST contents: {os.listdir(FRONTEND_DIST)}")

logger.info(f"GOOGLE_CLOUD_PROJECT: {os.getenv('GOOGLE_CLOUD_PROJECT')}")
logger.info(f"GOOGLE_CLOUD_LOCATION: {os.getenv('GOOGLE_CLOUD_LOCATION')}")
logger.info(f"MODEL_ID: {os.getenv('MODEL_ID')}")
# Do not log full API KEY for security, but check if it exists
api_key = os.getenv("GOOGLE_API_KEY")
logger.info(f"GOOGLE_API_KEY set: {bool(api_key)}")
if api_key:
    logger.info(f"GOOGLE_API_KEY prefix: {api_key[:8]}...")
logger.info("=======================================")
...
# Initialize google-genai client explicitly with API key if not using Vertex AI
...
if api_key and not use_vertex:
    ...
    genai_client = Client(api_key=api_key, http_options={'api_version': 'v1alpha'})
else:
    logger.info("Initializing GenAI Client with Default Credentials (Vertex AI)")
    genai_client = Client(vertexai=True, project=os.getenv("GOOGLE_CLOUD_PROJECT"), location=os.getenv("GOOGLE_CLOUD_LOCATION"))

# Define your runner with the explicit client
...
async def websocket_endpoint(
    ...
    logger.info(f"WebSocket connected: {user_id}/{session_id} (proactivity={proactivity}, affective_dialog={affective_dialog})")
...
if os.path.isdir(FRONTEND_DIST):
    # Explicitly serve index.html for the root path to ensure health checks pass
    @app.get("/")
