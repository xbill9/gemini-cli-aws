# Gemini Workspace for `mcp-fargate-rust-aws`
...
`mcp-fargate-rust-aws` is a streaming HTTP MCP server written in Rust, designed for deployment on Amazon ECS using **AWS Fargate**. It uses the `rmcp` library to provide tools to LLM agents via the Model Context Protocol.
...
- `make deploy` or `make fargate`: Deploys to Amazon ECS (AWS Fargate).
- `make logs`: Tail CloudWatch logs for the Fargate task.
- `make ecs-status`: Check Fargate service status.
- `make endpoint`: Get the public IP of the running task.
