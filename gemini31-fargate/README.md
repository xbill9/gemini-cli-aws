...
1.  **Initialize Environment**:
    ...
    make setup
    ```
    This runs `init.sh` to configure your Project ID and API Key, and `frontend.sh` to install frontend dependencies.
...
-   **Current Status**: All tests and linting are passing.
...
-   **Mock Mode**: Run `make mock` or `make run-mock` to start a mock backend that simulates Gemini's responses.
...
### Amazon ECS Fargate (AWS)

To deploy the system to Amazon ECS Fargate, ensure you have the [AWS CLI](https://aws.amazon.com/cli/) installed and configured with appropriate permissions.

1.  **Deploy to AWS**:
    ```bash
    make deploy
    ```
    This target runs `save-aws-creds.sh` and `deploy-fargate.sh` sequentially.
