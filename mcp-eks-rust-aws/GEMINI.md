# Gemini Workspace for `mcp-eks-rust-aws`
...
`mcp-eks-rust-aws` is a streaming HTTP MCP server written in Rust, designed for deployment on Amazon EKS (Elastic Kubernetes Service). It uses the `rmcp` library to provide tools to LLM agents via the Model Context Protocol.
...
*   **Deployment:** Amazon EKS (Managed Node Groups)
...
- `make deploy` or `make eks`: Deploys to Amazon EKS.
- `make cluster-create`: Create the EKS cluster using `eksctl`.
- `make cluster-delete`: Delete the EKS cluster.
- `make eks-status`: Check EKS deployment and service status.
- `make endpoint`: Get the LoadBalancer hostname for the service.
- `make logs`: Tail logs for the EKS pods.
