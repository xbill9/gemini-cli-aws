# Gemini Workspace for `mcp-lightsail-rust-aws`
...
`mcp-lightsail-rust-aws` is a streaming HTTP MCP server written in Rust, designed for deployment on Amazon Lightsail. It uses the `rmcp` library to provide tools to LLM agents via the Model Context Protocol.
...
- `make deploy-lightsail` or `make lightsail`: Deploys to Amazon Lightsail.
- `make lightsail-status`: Check deployment status.
