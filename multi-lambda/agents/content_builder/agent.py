...
async def log_before_agent(
    ...
    try:
                ...
                for part in callback_context.user_content.parts:
                    if part.text:
                        ...
                        if (
                            ...
                            and len(text) > 2
                            and len(text) < 100
                            and not any(e in text for e in ["🔍", "⚖️"])
                        ):
                            topic = text
                            break
        ...
        if findings and len(str(findings)) > 200:
            logger.info(
                f"Building course for topic '{topic}' (findings len: {len(str(findings))})"
            )
            # PASS ONLY FINDINGS AND TOPIC
            callback_context.user_content.parts = [
                genai_types.Part(
                    text=f"Target Topic: {topic}\n\nResearch Findings:\n{findings}"
                )
            ]
        else:
            logger.error("No findings found!")
            callback_context.user_content.parts = [
                genai_types.Part(text="ERROR: No research findings available.")
            ]
        return None
