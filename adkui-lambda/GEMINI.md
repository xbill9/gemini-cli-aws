...
*   `make deploy`: Primary command for AWS Lambda deployment (saves creds + deploys).
*   `make lambda-status`: Monitor the AWS Lambda deployment state.
*   `make endpoint`: Get the Lambda Function URL.
